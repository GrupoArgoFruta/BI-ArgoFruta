...
Para os dois últimos casos, a regra de ouro do `CONTRIBUTING.md` do repositório se aplica: mirar resultado idêntico para qualquer conjunto de dados.
...
A lista de pontos críticos conhecidos (publicação não atômica, ausência de `EXCEPTION WHEN OTHERS`, `DELETE` redundante pós-truncate, `BULK COLLECT` sem `SAVE EXCEPTIONS`) está em `ARCHITECTURE.md` e detalhada em `docs/STACK_MARGEM_BI.md`. Se a tarefa esbarrar em um desses pontos:
...
- Adicionar uma entrada datada no `CHANGELOG.md` distinguindo `✅ Confirmado` de `⚠️ Pendência`.
...
- Se for parte do futuro pipeline GCP (Dataflow/BigQuery/Looker Studio Pro da proposta Multiedro), confirmar primeiro com o usuário se o projeto já foi contratado — não assumir que a infraestrutura existe só porque está descrita no `ARCHITECTURE.md`.
