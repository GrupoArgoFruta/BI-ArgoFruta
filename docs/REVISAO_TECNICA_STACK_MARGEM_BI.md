...
Regra de ouro: toda proposta abaixo visa resultado idêntico para qualquer conjunto de dados. Onde a equivalência não é 100% garantível apenas pelo texto, a mudança é marcada [VALIDAR] e descrevo o risco em vez de recomendar cegamente. Ver `CONTRIBUTING.md` para a regra completa.

Log de progresso datado (o que foi diagnosticado/executado em cada rodada, distinguindo ✅ Confirmado de ⚠️ Pendência): ver `CHANGELOG.md`.
...
Nota de medição (05/07): VW_NOTAS_31 isolada custa ~3 min (~10% da carga). Não é o gargalo — este está na V9 (~17 min). Ver CHANGELOG.md. Otimizações adicionais aqui têm retorno limitado; priorizar a V9.
